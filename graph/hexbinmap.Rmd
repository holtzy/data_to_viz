---
myimage: "../img/section/MapHexbinSmall.png"
mydisqus: "hexbinmap"
mytitle: "Hexbin map"
output:
  html_document:
...
- The second definition refers to the utilisation of [2D density](file:///Users/yan/Dropbox/data_to_viz/graph/density2d.html) techniques. Think of a scatterplot where the X axis is the longitude and the Y axis is the latitude. The graphic area is divided in a multitude of hexagones and the number of data point in each is counted and represented using a color gradient. Note that here `no map boundaries is needed`. It requires only a `list of latitude and longitude`.
...
*Note on the first map*: You can learn more about this story [here](file:///Users/yan/Dropbox/data_to_viz/story/RegionWithValue.html). Data comes from [here](https://team.carto.com/u/andrew/tables/andrew.us_states_hexgrid/public/map). Code has been inspired from [this post](https://timogrossenbacher.ch/2016/12/beautiful-thematic-maps-with-ggplot2-only/) and [that one](https://rud.is/b/2015/05/15/u-s-drought-monitoring-with-hexbin-state-maps-in-r/).


*Note on the second map*: This map shows the geographic position of about 200k tweets containing the hashtags `#surf`, `#windsurf` or `#kitesurf`. Read more about it [here](file:///Users/yan/Dropbox/data_to_viz/story/GPSCoordWithoutValue.html).
...
Hexbin or grid map has an `advantage` over usual [choropleth maps](https://www.data-to-vize.com/graph/choropleth.html). In choropleths, a large polygon’s data looks more emphasized just because of its size, what introduces a bias. Here with hexbin, each region is represented equally dismissing the bias.  
...
Hexbin map uses hexagons to split the area in several parts and attribute a color to it. Note that it is possible to use square instead of hexagones, resulting in a [2D histogram map](file:///Users/yan/Dropbox/data_to_viz/graph/density2d.html).
