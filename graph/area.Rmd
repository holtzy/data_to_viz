...
Area chart is used to show the evolution of numeric variable. It is sometimes criticized for not optimizing the [data-ink ratio](https://www.infovis-wiki.net/index.php/Data-Ink_Ratio), a data visualization principle that checks that no inks is used for nothing on the chart. Indeed, removing the area under the curve would make a [line chart](https://www.data-to-viz.com/graph/line.html) that conveys the same information. In my opinion area chart make a very good work to show an evolution and the filled area makes the pattern even more obvious.
