...
`Three dimensional` objects are very popular but negatively affect the `accuracy` and `speed` at which one can interpret a graphic in most cases. Here is a review of the main graphic types involving 3d, with a discussion wether they are considered as bad practice or not.
...
- by construction, green bars in the bottom appear smaller than blue bars at the front for an equivalent value
- green bars in the bottom are hidden
...
<u>Workaround</u>: Don't do [pie charts](https://www.data-to-viz.com/caveat/pie.html).
...
***
The 3d scatterplot is another common use of 3d in datavisualization. This kind of chart takes 3 numeric variables as input, and represents it on the X, Y and Z axis.

The accuracy of 3D scatterplots is widely [discussed](https://www.gabrielaplucinska.com/blog/2017/8/7/3d-graphs). For instance, try to get the X, Y and Z coordinates of a specific point. You will realize that it is quite impossible since the human eye reads 3d badly. Thus, this graphic lacks accuracy.
...
***
If you have grid coordinates with a numeric variable for each position, you can represent your data using a surface plot. This kind of representation makes sense especially if the numeric value represents an altitude. In this case, we are actually building a 3D element that is a real 3d object, what makes sense in my opinion.
