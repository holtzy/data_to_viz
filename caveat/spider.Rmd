...
***
A `radar` or `spider` or `web` chart is a two-dimensional chart type designed to plot one or more `series` of values over `multiple quantitative variables`. Each variable has its own axis, all axis are joint in the center of the figure.

<br>

Let's consider the exam results of a student. He has a mark ranging from 0 to 20 for ten topics like math, sport, statistics and so on. The radar chart provides one axis for each topic. The shape allows to see for where did the student perform well.
...
In the previous chart only one serie is plotted, showing how one student performed. A common task is to compare several individuals. With only a few series it is possible to show every group on the same chart. Here it is quite obvious that Shirley globally performed better than Sonia, except in sport.
...
```
With more than two or three series, it is a good practice to use small multiple to avoid getting a too cluttered figure. Each student is represented on its own radar chart. It is easy to understand the feature of a specific individual, and looking for similarity in shapes allows to find student with the similar features.
...
- *Supporting the ranking* - In the above example, the lollipop plot is ordered. It allows to see instantly what was the topic with the best mark, and what is the topic ranking. This is more difficult with radar charts where there is no starts and ends.

- *Category order has a huge impact* - Radar chart reader will probably focus on the shape observed. This can be misleading since this shape highly depends on the order of category order around the plot. See these charts made using the same data, but swaping category orders:
```{r, warning=FALSE, message=FALSE, fig.align="center", fig.height=6, fig.width=10}
# Create data: note in High school for Jonathan:
set.seed(7)
data <- as.data.frame(matrix( sample( 2:20 , 10 , replace=T) , ncol=10))
...
- *About scales* - Radar charts display the value of several quantitative variables, all represented in an axis. In the previous example, the same scale and the same unit is shared by all variables (a mark ranging from 0 to 20). It is also possible to display variables that are completely different. In this case, don't forget to show an abvious scale for each: the reader will expect the same scale otherwise.
...
- *Over evaluation of differences* - the area of a shape in a radar chart also increases a square of the values as opposed to linearly, which could lead viewers to think that small changes are more significant than they might actually be. In the following example, the student had a 7 in every topic on the left, 14 on the right. However the right area is bigger than twice the left area.
...
- If you have a single serie to display, and that all quantitative variables have the same scale, then use a barplot or a lollipop plot, ranking the variable:
...
- If you have a two series to plot, you can still play with barplot and lollipop plots. Here is an example with 2 series. It focus on the first student (dark) and allows to see how is friend (light) performed in comparison.
```{r, warning=FALSE, message=FALSE, fig.height=5, fig.width=7, fig.align="center"}
# Create data: note in High school for Jonathan:
set.seed(1)
...
- If you have a lot of series to plot, or if your variables do not have the same scale, then the best option is probably to switch to a parrallel coordinate plot.
